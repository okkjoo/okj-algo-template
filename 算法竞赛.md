...
### [用筛法求之N内的素数](https://www.dotcpp.com/oj/problem1084.html)
...
贪心：从最左侧开始遍历，如果该位置硬币状态与目标不同，就翻动该位置和该位置后面的两枚硬币。
...
### 单词分析|蓝桥杯|max_element|lambda 表达式
...
[数位DP](https://blog.csdn.net/AC__dream/article/details/123168715)

```cpp
//数位dp的特性：需要求解的内容与数的组成有关而与数的大小无关。
//第一步：把数按位拆分
//第二部：进入记忆化搜索
//状态数组的设定：
//对于此题：dp[i][j][k] 代表的是第i个数  此时已经有了1的个数  此时的数是0还是1. 
```
...
//`dp[i][j][cnt][k]`:`在(i, j) 这个点拿了 cnt个物品，其中价值最大值为
// k`,方案数为dp的值
...
int main() {
  ...
  FOR(i, 1, n + 1) {
    FOR(j, 1, m + 1) {
      cin >> a[i][j];
      a[i][j]++;  //所有物品价值都+1   即所有物品的价值范围为[1,13]
    }
  }

  //在边界进行初始化
  dp[1][1][0][0] = 1;        //起点拿的方案数为1
  dp[1][1][1][a[1][1]] = 1;  //起点不拿的方案数也为1
  //开始枚举
  FOR(i, 1, n + 1) {
    FOR(j, 1, m + 1) {                 //遍历(1,1)到(i,j)这个点
      if (i == 1 && j == 1) continue;  //起点跳过
      FOR(cnt, 0, k + 1) {             //枚举手中物品的数量
        FOR(v, 0, 14) {  //遍历已经拿到的最大价值 [0,13]
          //在该点不拿物品
          ...
          //若取的话:
          // 1. 手中的物品总数量一定大于0
          // 2. 当前循环到的物品价值一定等于此处位置的物品价值
          if (cnt > 0 && v == a[i][j]) {
            //枚举 上一步 || 左一步 的物品最大值,最大值小于当前值的就可以加进来
            FOR(s, 0, a[i][j]) {
              dp[i][j][cnt][v] =
...
### 洛谷 [P2015 二叉苹果树](https://www.luogu.com.cn/problem/P2015)|树型背包

> 给定需要保留的树枝数量，求出最多能留住多少苹果。

- [树型DP入门详解](https://www.cnblogs.com/hanruyun/p/9788170.html)

>  这道题有一个隐含的条件，当==某条边被保留下来时，从根节点到这条边的路径上的所有边也都必须保留下来==
>
> 设`f[u][i]`表示 *u*的子树上保留i 条边，至多保留的苹果数目
>
> 那么状态转移方程也就显而易见了
>
> ![image-20220127165719570](https://gitee.com/okkjoo/image-bed/raw/master/imgs/image-20220127165719570.png)
>
> *`u`*表示当前节点，`v`是`u`的一个子节点，`sz[u]`表示`u`的子树上的边数，`q`就是题目中要求的最多保留边数
>
> **还要倒序枚举，因为是 01 背包，有可能会拿多。**
...
int f[N][N];  // f[j][k] 表示以第j个点为根的子树选取k个边能保留的最大权值
...
void dfs(int u, int fa) {
  for (int i = head[u]; ~i; i = e[i].nxt) {
    ...
    //在归中操作
    sz[u] += sz[v] + 1;  //以u为根的子树的边数 = 以v为根的子树的边相加 + 1(uv)
    //注意jk范围
...
int main() {
  memset(head, -1, sizeof head);  //千万记得初始化head
  cin >> n >> m;
  FOR(i, 1, n) {
    cin >> u >> v >> w;
    add(u, v, w);
...
### [1220. 生命之树|树型DP](https://www.acwing.com/problem/content/1222/)

要求求出S最大的值，S其实是一个**联通块**。所以就是求所有联通块里面值最大的那个。——没这么麻烦的树型DP

f[u]表示的是以u为根节点的连通块的sum的最大值
...
###  洛谷[P2014 [CTSC1997\] 选课](https://www.luogu.com.cn/problem/P2014)|树型背包

先构建一个虚拟头——虚拟课程：没有先行课的先修课，则包含N个结点的森林就转换为了 N+1 个结点的树，其中结点0为根节点。（因为一开始可能是无根的，不好操作）

设 `dp[x][t]`为以x为根的子树中，选取 t 门课能获取的最高学分，设x的子节点集合为 `Son(x)`,子节点个数为 `p = |Son(x)|`，选择x这门课后，对于x的每个子节点y，以y为根的子树中选修 t -1 门课程，并且在满足 `t - 1` 门课程基础上获得最多的学分。
...
通常用的都是 堆式储存法 ，即编号为 k  的节点的左儿子编号为 `2k` ，右儿子编号为 `2 k + 1`，父节点编号为 `k/2` ，用 位运算 优化一下，以上的节点编号就变成了 `k<<1 , k<<1|1 , k>>1` 
...
2. 修改这一段区间的所有点。
把修改区间分解成一段一段的。但问题来了：我们怎样修改这些区间呢？
...
https://blog.csdn.net/qq_35577488/article/details/117809079

## 数论

### [1216. 饮料换购](https://www.acwing.com/problem/content/1218/)|数学
...
### [1211. 蚂蚁感冒](https://www.acwing.com/problem/content/1213/)|数学

> 首先我们必须要明白两只蚂蚁相撞掉头可以看作时一只蚂蚁穿过了另一只蚂蚁，因为相撞之后两只蚂蚁都感冒了，掉不掉头其实无所谓，毕竟都感冒了，这样的话这题就简单多了。
>
> 一般情况：
> 感冒蚂蚁右边的往左边走、左边的往右边走  就是会感染的 `ans = right + left+1` ，`right` 表示`在其右边的往左边走的蚂蚁数量`，`left`表示`在其左边的往右边走的蚂蚁数量`，`1`就是其本身
>
> 特殊情况：
> 第一只蚂蚁向左走的时候，如果第一只蚂蚁左边没有向右爬行的蚂蚁，由于爬行速度相同，所以不管第一只蚂蚁右边有多少向左爬行的，其右边的蚂蚁永远不可能被感染。同理，当第一只蚂蚁向右走的时候，如果第一只蚂蚁右边没有向左爬行的蚂蚁，其左边也永远不可能感染。```
...
### [AcWing|1205. 买不到的数目](https://www.acwing.com/problem/content/1207/)|数论|结论题||DP

> 假设m n 为 4、7，那么无非三种情况：
>
> 1. 单纯用4
> 2. 单纯用7
> 3. x 个 4 ，y 个 7
>
> 那么重点思考就是第三种情况：很明显，这样的话就是 原来的数减去 `4x`后可以整除 7 ，此时就说明它可以由4和7组成。==>`while((k-4)%7!=0&&k>0)k-=4;`
...
int main() {
  int n, m;
  scanf("%d%d", &n, &m);
  int k = n * m; //题目保证有解的情况下，由数论得出的结论，很多题解都用到了，证明如下
  while (k) {
    int t = k;
    while (t % m != 0 && t - n > 0) t -= n;
    if (t % m != 0 && k % n != 0 &&
        k % m != 0)  //这一步就是判断了1、2、3 三个条件
    {
      printf("%d", k);
      break;
    }
...
粗略证明：`knm+1 ~ (k+1)nm`就可以由`(0 ~ nm )+(0 ~ nm)+...`获得，那么就感觉 总能叠加到那种效果

甚至还有一个`O(1)`的方法，直接返回`(a - 1)(b - 1) - 1`额 很明显 这种数学方法，我也不太懂。

但是还有一个更为通用暴力的方法：DP，但前提也是有那个结论
...
bool dp[1000000]; // 数据范围：1000*1000
int main() {
    cin >> n >> m;
    dp[0] = true;
    minn = min(n, m);
...
### scnu.oj[|cy老师能否减肥成功](http://csoj.scnu.edu.cn/problem/S0280)|矩阵快速幂

> 从某个起点走到某个终点刚刚好走了x*条边（也就是一条长度为x*的不一定简单的路径），称为一种cy老师接受的方案
> ——长度为x的路线——邻接矩阵的x次幂

先处理矩阵base（就是读入的地图），对矩阵base的x次方，对角线上的数乘以(a+b)，其他乘以a，累加答案。
...
int a, b, w, x;
vector<ll> vv(105, 0);
vector<vector<ll>> g(105, vv);    //初始矩阵
vector<vector<ll>> ans(105, vv);  // x次方后的结果

//矩阵快速幂
void qpow() {
  int xx = x;
  FOR(i, 0, 105) ans[i][i] = 1;  //初始化为单位矩阵，而不是全1
  while (xx) {
    if (xx & 1) {
...
### scnu.oj|[取石子](http://csoj.scnu.edu.cn/contest/45/problem/5)|nim游戏

只要将各堆石子数异或起来，不为0就代表先手必胜。
...
### scnu.oj|[csome树](http://csoj.scnu.edu.cn/problem/S0282)|树形背包

`dp[i][j]`表示在i结点，选中j个的状态。
...
### scnu.oj|梦话接龙|欧拉路径+并查集

对每行的字符串，首字符为a，尾字符为b，则该字符串相当于有一条 a->b 的有向边。

则，出度等于入度的点当然没问题，则只需要考虑出度不等于入度的点。

可以先写一道[欧拉路径模板题](https://www.luogu.com.cn/problem/P7771)
...
stack<int> st;
vector<int> G[N];
int del[N];  // del[now] 表示 G[now][1,2……,del[now]-1]
             // 都已经被标记访问过，下一次要从G[now][del[now]]开始访问
int n, m, u, v;
void dfs(int now) {
...
int main() {
  ...
  int S = 1, cnt[2] = {0, 0};  //记录起点和出入度差为1的数目
  bool flag = 1;               // 1表示所有结点的入度都等于出度
  FOR(i, 1, n + 1) {
