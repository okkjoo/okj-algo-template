...
## L2-025 分而治之 (25 分)|图的遍历|并查集|连通分量
...
- 但是他也没说哪个门是入口
  - 无根变有根,判断哪个是入口，然后DFS，找到这棵树的最大深度。
    - 如果想判断哪个是入口：读入地图的时候，没有通往的点就是起点（读入时“入度”为0的）
  - 也可以不判断：直接 BFS，根据==结果唯一==以及BFS的特性：在产生新的子结点时，`深度越小的结点越先得到扩展`，即先产生它的子结点
    - ——留到队列中的最后一个就是离得最远的，也就是答案
    - 值得注意的是 一定要从1开始不然会错一个测试点：只有1一个的
  ...
  FOR(i, 0, n) {
    ...
    if (s == "Push") {
      ...
      if (stk.size() == 0)
        ...
        int sz = stk.size();
        auto it = lower_bound(v.begin(), v.end(), stk[sz - 1]);//找到有序栈中的位置
        v.erase(it);//删除
        cout << stk[sz - 1] << endl;//删除正常栈
        stk.pop_back();
      }
...
**pre 记录路径中上一个结点，然后从终点开始往回搜，得到路径，在 `dijkstra`中真的是太常用了**
